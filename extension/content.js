  ...
  `;
    ...
    function boot(){
          ...
          try {
              ...
              if (!jobData.title || jobData.title === 'Unknown Job') {
                  ...
                  if (jobCard) {
                      ...
                      // Look for company name within this specific job card
                      ...
                      for (const selector of companySelectors) {
                          ...
                          if (companyEl && companyEl.innerText.trim() && companyEl.innerText.trim() !== 'MANUAL') {
                              const companyText = companyEl.innerText.trim();
                              console.log('[Oppli] ✅ Found company with selector:', selector, companyText);
                              
                              // Verify this looks like a real company name (not too short, not generic)
                              if (companyText.length > 2 && !/^(logo|apply|save|share|remote|hybrid|full-time|part-time|contract)$/i.test(companyText)) {
                  ...
                  if (!jobData.companyName || jobData.companyName === 'MANUAL') {
                      ...
                      if (mainJobContainer) {
                          ...
                          // Look for company name within the main job container only
                          const companySelectors = [
                              ...
                              'h1 + div a[href*="/company/"]',
                              'h1 + div a[href*="/school/"]',
                              '.jobs-unified-top-card__subtitle a[href*="/company/"]',
                              '.jobs-unified-top-card__subtitle a[href*="/school/"]'
                          ];
                          
                          for (const selector of companySelectors) {
                              const companyEl = mainJobContainer.querySelector(selector);
                              if (companyEl && companyEl.innerText.trim() && companyEl.innerText.trim() !== 'MANUAL') {
                                  const companyText = companyEl.innerText.trim();
                                  console.log('[Oppli] ✅ Found company with selector:', selector, companyText);
      ...
      function scrapeJobDom(topCard) {
          ...
          // 1) any explicit company link (company/school)
          const link =
              topCard.querySelector("a[href*='/company/']") ||
              topCard.querySelector("a[href*='/school/']") ||
              topCard.querySelector("a[data-control-name*='company']") ||
              topCard.querySelector("a.topcard__org-name-link");
          if (link) companyName = (link.innerText || link.textContent || "").trim();

          // 2) common wrappers across AB variants
          if (!companyName) {
              companyName =
                  textIn(topCard, [
                      ".jobs-unified-top-card__company-name a",
                      ...
                      // NEW: many regions render this
                      ".artdeco-entity-lockup__subtitle a",
                      ".artdeco-entity-lockup__subtitle span",
                      ".app-aware-link[href*='/company/']",
                      ".app-aware-link[href*='/school/']"
                  ]) || "";
          }

          // 3) generic "primary subtitle" text fallback
          if (!companyName) {
              const primary = Array.from(topCard.querySelectorAll("div,span"))
                  .find(n => /\bsubtitle\b/i.test(n.className) && /\bprimary\b/i.test(n.className));
              if (primary) {
                  let t = (primary.innerText || primary.textContent || "").trim();
                  companyName = t.split("·")[0].split("\n")[0].trim();
              }
          }
